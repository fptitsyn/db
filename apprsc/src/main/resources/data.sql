...
$$ LANGUAGE plpgsql;
------------------------------------------------------------------------------------------------------------
-- Функция возвращает всех сотрудников (и их должность) которые работают в определенном офисе (date_of_dismissal - пустая)
--SELECT * FROM get_employees_by_location(1); -- где 1 - искомый location_id
CREATE OR REPLACE FUNCTION get_employees_by_location(p_location_id bigint)
RETURNS TABLE (
    employee_id bigint,
    full_name_with_position text
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        e.employee_id,
        CONCAT_WS(' ', e.last_name, e.first_name, e.middle_name) || ' (' || st.position || ')' AS full_name_with_position
    FROM
        employees e
    INNER JOIN
        employees_moving em ON e.employee_id = em.employee_id
    INNER JOIN
        staffing_table st ON em.staffing_table_id = st.staffing_table_id
    WHERE
        em.date_of_dismissal IS NULL
